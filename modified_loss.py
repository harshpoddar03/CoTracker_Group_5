...
def sequence_loss(flow_preds, flow_gt, vis, valids, gamma=0.8):
    ...
    for j in range(len(flow_gt)):
        ...
        for i in range(n_predictions):
            ...
            # i_loss =  torch.norm(flow_pred - flow_gt[j], dim=-1)
            # print(f"i_loss: {i_loss.shape}")
            # print(f"flow_pred shape: {flow_pred.shape}")
            # print(f"flow_gt shape: {flow_gt[j].shape}")
            # print(f"i_loss shape: {i_loss.shape}")  # B, S, N, 2
            i_loss = torch.mean(i_loss, dim=3)  # B, S, N
            # print(f"III: {i_loss[:,:,10]}, {(flow_pred - flow_gt[j]).abs()[:,:,10]}")
