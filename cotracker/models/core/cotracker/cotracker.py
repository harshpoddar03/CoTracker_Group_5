...
class CoTracker2(nn.Module):
    ...
    def forward_window(
        ...
        # print(f"Position embedding shape_initial: {self.pos_emb.shape}")
        # print(f"Coords_frame1: {coords[:,0]}")
        # print(f"Coords: {coords}")
        # print("HEYYYYYYY")
        # print(f"KP embedding before: {self.kp_emb.shape}")
        kp_emb_repeated = self.kp_emb.unsqueeze(0).repeat(B,1,1,1).reshape(-1,1,self.input_dim)
        # print(f"KP after: {self.kp_emb.shape}")
        # print(f"Coords shape: {coords.shape}")
        # print(f"Input dimension: {self.input_dim}, KP embedding before: {self.kp_emb.shape}")
        sampled_pos_emb = (
        ...
        for __ in range(iters):
            ...
            # print(f"Time embedding shape: {self.time_emb.shape}")
            # print(f"Transformer input shape: {transformer_input.shape}")
            # print(f"Position embedding shape: {sampled_pos_emb.shape}")
            # print(f"Key point embedding: {self.kp_emb.shape}")
            if(self.skip):
    ...
    def forward(self, video, queries, iters=4, is_train=False, is_online=False):
        ...
        # print(f"Hey, Video shape: {video.shape}, queries shape: {queries.shape}")
        ...
        # print(f"Queries_coords_init shape: {queried_coords.shape}")
        ...
        # print(f"Queries_coords shape: {queried_coords.shape}")
        ...
        # print(f"Indices here : {indices}")
        for ind in indices:
            # print(f"Indices here : {ind}")
            ...
            # print(f"Hey coords shape : {len(coords)}, vis shape : {len(vis)}")
            # print(f"Hey coords shape : {(coords[0].shape)}, vis shape : {vis[0].shape}")
            S_trimmed = T if is_online else min(T - ind, S)  # accounts for last window duration

            # print(f"S_trimmed: {S_trimmed}")
        ...
        if is_train:
            # print(f"queried_frames: {queried_frames}")
            # print(f"queried_frames_shape: {queried_frames[:, None].shape}")
            # print(f"{torch.arange(0, T, device=device)[None, :, None]}")
